...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
    ...
    const XeonBotInc = makeWASocket({
      ...
      browser: [ 'Chrome (Linux)', 'Chrome', '1.0.0'], // for this issues https://github.com/WhiskeySockets/Baileys/issues/328
     auth: {
         creds: state.creds,
   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +6283137499954")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number \nFor example: +6283137499954 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
